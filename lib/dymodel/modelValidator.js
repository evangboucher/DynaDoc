...
function parseSingleSchema(schema, payload, index) {
    if (schema.key) {
        ...
        if (schema[SCHEMA]) {
            //There is a schema with this key. Pass in a new index to start from.
            parseSingleSchema(schema[SCHEMA], payload[schema.key], 0);
        }
    }
...
/**
Parses a joi schema and returns a DyModel object.

Example of a DyModel object:
{
    "CustomerID": {
        "type": "S"
    },
...
**/
validator.parseJoiSchema = function parseJoiSchema(schema, modelName) {
    ...
    dymodelObject.TableName = modelName;
