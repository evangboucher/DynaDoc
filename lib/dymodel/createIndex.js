...
**/
function generateMainIndexKeys(indexPayload, indexObject,readCapacity, writeCapacity) {
...
function addRangeKey(payload, indexPayload, indexObject, dyModel) {
    ...
    if (indexObject[INDEX_CREATION_RANGE_KEY]) {
        ...
        indexPayload.KeySchema.push({"AttributeName":indexObject[INDEX_CREATION_RANGE_KEY],"KeyType": TYPE_KEY_RANGE});
...
@param writeCapacity (Integer): The write throughput for DynamoDB index.
**/
CreateIndexHelper.addGlobalIndex = function addGlobalIndex(payload, indexObject, dyModel, readCapacity, writeCapacity) {
...
@param dyModel (Object): The DyModel that describes the table.
**/
CreateIndexHelper.addLocalIndex = function addLocalIndex(payload, indexObject, dyModel) {
