...
**/
function DyModel(modelName, model, dynamoDB, readCapacity, writeCapacity) {
...
function setThroughput(tableCreationPayload, readCapacity, writeCapacity) {
    ...
    tableCreationPayload.ProvisionedThroughput = {"ReadCapacityUnits":readCapacity, "WriteCapacityUnits":writeCapacity};
}
...
**/
DyModel.prototype.ensureGlobalIndex = function ensureGlobalIndex(hashKey, rangeKey, readCapacity, writeCapacity, indexName, options) {
    //These assertions will fail if the user passes in bad input.
    Joi.assert(readCapacity, Joi.number().integer().min(this.MIN_THROUGHPUT).max(this.MAX_THROUGHPUT));
    Joi.assert(writeCapacity, Joi.number().integer().min(this.MIN_THROUGHPUT).max(this.MAX_THROUGHPUT));
    ...
    if (this.createLock === true) {
        ...
        CreateIndexHelper.addGlobalIndex(this.tablePayload, indexObject, this.dyModel, readCapacity, writeCapacity, indexName, true);
        return;
    }
    //Global Indexes are the only indexes that can be updated or created after the table is generated.
    var indexObject = CreateIndexHelper.createIndexObject(hashKey, rangeKey, options);
...
**/
DyModel.prototype.ensureLocalIndex = function ensureLocalIndex(rangeKey, indexName, options) {
    if (this.createLock === true) {
        throw Util.createError('DynaDoc: ensureLocalIndex was called after the table was created!');
    }
    var indexObject = CreateIndexHelper.createIndexObject(this.dyModel[Util.PRIMARY_INDEX_PLACEHOLDER][Util.constants.INDEX_PRIMARY_HASH], rangeKey, options);
...
/**
Smart Update will update a item in the database with the given key
and only change the given items. Update always uses the primary index.

smartUpdate requires you to use DyModel (DynaDoc's model feature) in
order to function. Otherwise you must use the standard promise based
update function.
...
@param newObject (Object): The new object that will take the place of the old
   ...
   what was already in dynamoDB. The newObject must contain the primary key!
   Primary key (both hash or range) is never updated or changed in any way.
...
**/
/*
DyModel.prototype.smartUpdate = function smartUpdate(newObject, options) {
    var d = Q.defer();
    var payload = {};
    payload.TableName = this.modelName;
    SmartUpdateHelper.smartUpdateAttributeUpdates(payload, this.dyModel, newObject, options);

    this.dynamoDoc.update(payload, function(err, res) {
        if (err) {
            d.reject(err);
            throw err;
        }
...
**/
DyModel.prototype.buildSmartUpdate = function buildSmartUpdate(newObject, options) {
