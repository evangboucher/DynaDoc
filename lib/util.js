...
util.getSavedQuery = function getSavedQuery(settings, indexName, action) {
    ...
    if (savedQueries.hasOwnProperty(queryHash)) {
...
/**
@TODO remove later if this is deemed not needed for production.
Returns data type for basic data types.
**
function getDataTypeBasic(variable) {
    if (typeof variable === TYPE_OF_NUMBER) {
        return DYNAMO_NUMBER;
    } else if (typeof variable === TYPE_OF_STRING) {
        return DYNAMO_STRING;
    }else if (typeof variable === TYPE_OF_BOOLEAN) {
        return DYNAMO_BOOL;
    }
...
/**
Returns the DynamoDB data type for the variable passed in.
@param variable (any): The variable to check the type of.
@returns String: DynamoDB data type string. If not match, defaults to DynamoDB String
**
@TODO remove later if this is deemed not needed for production.
util.getVariableDataType = function getVariableDataType(variable) {
    if (Array.isArray(variable)) {

        return DYNAMO_LIST;
    } else if (typeof variable === TYPE_OF_OBJECT){
        return DYNAMO_MAP;
    } else {
        return getDataTypeBasic(variable);
    }
    return DYNAMO_STRING;
}
*/
