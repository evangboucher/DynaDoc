...
Note: You must provide table names in the params.
**
DynaDoc.prototype.batchWrite = function batchWrite(params, options) {
    var d = Q.defer();

    //Here lets add the options they passed in.
    if (options) {
        //Add the options.
        Util.addOptionsToPayload(params, options);
    }
...
DynaDoc.prototype.createSet = function createSet(params) {
    return this.dynamoDoc.createSet(params);
}
...
DynaDoc.prototype.queryOne = function queryOne(indexName, keyConditionExpression, expressionAttributeValues, expressionAttributeNames) {
        ...
        return this.dynamoDB.queryAsync(payload);
    }
    /**
    ...
    **/
DynaDoc.prototype.query = function query(indexName, hashValue, rangeValue, action, options) {
    if (arguments.length <= 1) {
        //We should throw some error because the user is miss using the function.
        throw Util.createError('Not enough arguments (#Args: ' + arguments.length + ' Expected (2-5)) for smartQuery!');
    }
    ...
    switch(arguments.length) {
        ...
        case 3:
            ...
            break;
        //4 or 5 means they either have everything, but options, or included options.
        case 4:
        case 5:
        ...
        default:
            //We should throw some error because the user is miss using the function.
            throw Util.createError('Too many arguments (#Args: ' + arguments.length + ' Expected (2-5)) for smartQuery!');
            break;
    }
    ...
    return this.dynamoDoc.query(payload);
}
...
@param options (Object; Optional): The DynamoDB options for a batchWrite call.
    ...
    - ReturnItemCollectionMetrics: 'SIZE' | 'NONE'
**/
DynaDoc.prototype.batchWrite = function batchWrite(arrayOfTableNames, putItemsObject, deleteItemObject, options) {
    ...
    return this.dynamoDoc.batchWrite(payload);
}
...
DynaDoc.prototype.smartBatchGet = function smartBatchGet(arrayOfTableNames, batchGetKeyObject) {
    ...
    return this.dynamoDoc.batchGet(payload);
}
...
DynaDoc.prototype.deleteItem = function deleteItem(key, options) {
    ...
    return this.dynamoDoc.delete(payload);
}
...
DynaDoc.prototype.updateItem = function updateItem(params) {
    ...
    return new Promise(function (resolve, reject) {
        that.dynamoDoc.update(params, function(err, res) {
            if (err) {
                reject(err);
                return;
            }
...
DyModel.prototype.updateTable = function updateTable() {
    ...
    var that = this;
    return new Promise(function (resolve, reject) {
        that.dynamoDB.updateTable(this.tablePayload, function(err, res) {
            if (err) {
                reject(err);
                return;
            }
...
DynaDoc.prototype.addFunction = function addFunction(methodName, method) {
    ...
    if (this.dyModel.hasOwnProperty(methodName)) {
        //Method name already exists. Throw error.
        throw Util.createError('addFunction(): Method name: ' + methodName + ' already exists in DynaDoc!');
    }
