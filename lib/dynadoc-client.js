...
DynaDoc.prototype.query = function query(indexName, hashValue, options) {
    if (arguments.length < 2 || arguments.length > 3) {
        ...
        throw Util.createError('Improper amount of arguments (#Args: ' + arguments.length + ' Expected (2-3)) for smartQuery!');
    }
    var rangeValue = undefined;
    var action = "=";
    console.log('Query(): The options is:');
    console.log(JSON.stringify(options, null, 4));
    if (options) {
        if (options.hasOwnProperty(CONSTANTS.KEY_RANGE_VALUE)) {
            console.log('Query(): Utilizing a rangeValue! the value is: ' + options[CONSTANTS.KEY_RANGE_VALUE]);
    ...
    console.log('Query(): The rangeValue is: ' + rangeValue + " the action is: " + action);
    ...
    console.log('Query(): The payload is: ');
